...
def main():
    ...
    scraping2 = WebScrapingBS4(link=f'{config.NOTICIAS_ANIME}')
    ...
    with Database() as cur:
        for noticia, link in zip(noticias, links):
            ...
            cur.execute("""SELECT noticia FROM noticias_animes
                        WHERE noticia = %s""", noticia)
            varificao = cur.fetchone()

            cur.execute("""DELETE FROM noticias_animes WHERE DATEDIFF(CURDATE(), date_cadastro) > 3;
            """)
            ...
            cur.execute("""INSERT INTO noticias_animes(noticia, link) VALUES (%s, %s)""", (noticia, link))
